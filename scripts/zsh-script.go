...
var ZshCompletionScript = `local kubectlPlugins=(%s)

declare -A pluginDescription

declare -A cobraSupported
...
_kubectl()
{
    ...
    if [[ ${CURRENT} == "2" ]]; then
        ...
        do
            if [[ ! -z ${pluginDescription[$plugin]} ]]; then
                completions+="$plugin:${pluginDescription[$plugin]}"
            else
                completions+="$plugin:"               
            fi
        done
